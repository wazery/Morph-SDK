...
void OgreMaxUtilities::LoadBoundingVolume(const TiXmlElement* objectElement, BoundingVolume& volume)
{
    ...
    volume.radius = OgreMaxUtilities::GetRealAttribute(objectElement, "radius", 0);
    volume.size.x = OgreMaxUtilities::GetRealAttribute(objectElement, "height", 0);
...
bool OgreMaxUtilities::SetDefaultLighting(SceneManager* sceneManager, UpAxis upAxis)
{
    ...
    if (defaultLightingNeeded)
    {
        ...
        else if (sceneManager->getAmbientLight() == ColourValue::White)
            sceneManager->setAmbientLight(ColourValue((Real).7, (Real).7, (Real).7));
    }
